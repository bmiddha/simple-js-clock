...
const ctaTrainApiKey = process.env.CTA_TRAIN_API_KEY;
function getCtaTrain(trainStation: string): Promise<{}> {
    return new Promise((resolve, reject) => {
        const trainUrl = "https://lapi.transitchicago.com/api/1.0/ttarrivals.aspx?key=" + ctaTrainApiKey + "&mapid=" + trainStation + "&max=5&outputType=JSON";
